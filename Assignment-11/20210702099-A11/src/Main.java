...
public class Main {
	...
	public static void main(String[] args) {
		...
		ArrayList<Calculable> CalculableArray = new ArrayList<Calculable>();
		
		// !!!!!!!!!! INPUTS FOR DEBUGGING, UNCOMMENT IF NEEDED !!!!!!!!!!
		
		CalculableArray.add(new Reservation("Tunceli", 		"Hilton", 		"Jan", 		2, 		9));		// 1
		CalculableArray.add(new Reservation("Şırnak", 		"Ramada", 		"Feb", 		1, 		12));		// 2
		CalculableArray.add(new Reservation("Hakkari", 		"Sheraton", 	"March", 	4, 		17));		// 3
		CalculableArray.add(new Reservation("Diyarbakır", 	"Radisson", 	"April", 	5, 		28));		// 4
		CalculableArray.add(new Reservation("Kırklareli", 	"Sheraton", 	"May", 		10, 	12));		// 5
		CalculableArray.add(new Reservation("Batman", 		"Sheraton", 	"June", 	2, 		20));		// 6
		CalculableArray.add(new Reservation("İzmir", 		"Hilton", 		"July", 	7, 		30)); 		// 7
		CalculableArray.add(new Reservation("Edirne", 		"Rixos", 		"Aug",		9, 		5));		// 8
		CalculableArray.add(new Reservation("Iğdır", 		"Bomonti", 		"Sept", 	8, 		5));		// 9
		CalculableArray.add(new Reservation("Muğla", 		"Dosco", 		"Oct", 		29, 	31));		// 10
		CalculableArray.add(new Reservation("Mardin", 		"Dedeman", 		"Nov",		7, 		5));		// 11
		CalculableArray.add(new Reservation("Aydın", 		"Kaynarkazan",  "Dec",		9,		5));		// 12
		...
		// Button listener for "Multithreading Search"
		mainframe.getMultithreadSearch().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				mainframe.setTerminalText("");
				if(Reservation.totalNumberOfReservations < 8) {
					JOptionPane.showMessageDialog(mainframe, "Please enter at least 8 reservations - now it is only " + Reservation.totalNumberOfReservations);
				}
				else {
					String hotelName_multithread = JOptionPane.showInputDialog(mainframe, "Enter Hotel Name:", JOptionPane.INFORMATION_MESSAGE );
					mainframe.setTerminalText("Reservation IDs for " + hotelName_multithread + ":\n");
					
					final int SERVICE_PER_THREAD = 4; // determines how many Services are to be handled by a single Thread (arbitrarily four)
					int dividedChunks = Math.floorDiv(Reservation.totalNumberOfReservations, SERVICE_PER_THREAD); // number of threads needed (if the remainder isn't zero, need 1 more thread)
					int totalServices = Reservation.totalNumberOfReservations;
					int remainderServices = totalServices - (dividedChunks * SERVICE_PER_THREAD); // if greater than 0, we need an extra thread for the remaidner
					
					ExecutorService executorService;
					if(remainderServices < 0)
						executorService = Executors.newFixedThreadPool(dividedChunks + 1);
					else 
						executorService = Executors.newFixedThreadPool(dividedChunks + 1);
					...
					int iterator;
					for(iterator = 1; iterator < totalServices + SERVICE_PER_THREAD - 1; iterator += SERVICE_PER_THREAD) {
						executorService.submit( new MultithreadSearch( (ArrayList<Calculable>) CalculableArray, mainframe, hotelName_multithread, iterator-1, iterator+2) );
					}
						
					
					// Although I have assigned a thread per 4 service. If nr of services isnt divided by 4 evenly, have the remainder of services (1 <= x <= 3) checked by another thread
					if(remainderServices > 0)  
						executorService.submit( new MultithreadSearch( (ArrayList<Calculable>) CalculableArray, mainframe, hotelName_multithread, iterator-1, iterator + remainderServices - 2));
